...
| Student Reports | Student manages weekly reports. | Calendar from listReportingPeriods, including weeks with no report, due instant/timezone and missing/late state; Draft/Submitted/Revision Requested/Approved state and latest update. | Start report for available week, open draft/history. | Empty; no Active placement; active report for week; inactive placement read-only. | Placement Overview → Report Editor or Report Detail. |
| Report Editor | Student saves, submits, or revises one weekly report. | Server-provided reporting-period selector/identity (future weeks disabled), accomplishments, challenges, next-week plan, optional attachments reloaded from saved draft, private draft state, prior reviewer feedback when revising; revision stays Revision Requested until resubmission. | Save draft, submit, revise/resubmit, cancel. | Required-content validation; draft private; duplicate-week prevention; revision-requested; Active-only; upload/submit error. | Reports → Report Detail after submit; back preserves draft. |
| Student Report Detail | Student reads the report and retained history. | Current submitted content, attachments, review outcome/feedback, chronological submitted-version and review history joined by reportVersionId; only this Student sees saved revision draft content. | Revise when Revision Requested; return to reports. | Draft private; Submitted awaiting review; Revision Requested; Approved; inactive placement read-only. | Reports/notification → Report Editor or Reports list. |
...
| Student Profile | Student maintains application-ready profile. | Identity/contact and education editor, Skills & Preferences subpages, reusable Documents file list. | Edit/save identity/education; add/change/remove skill and optional proficiency; set preferences; upload/replace/remove reusable documents. | Loading/error; reload skills through getMySkills with stable IDs and optional proficiency before enabling replacement save; saved confirmation; field/upload errors; no skills/documents. | Student nav; Profile links to Skills & Preferences/Documents. |
...
| Posting Editor | Staff/Admin creates or edits a posting. | Company, title, description, category, location, work arrangement, duration, deadline, openings, required/optional skill groups with explicit None choices (empty groups allowed), optional responsibilities/benefits. | Save Draft, update Draft, attempt Publish. | Field errors; publish validation names every missing required publishing field; saved Draft; error. | Posting List/detail → posting detail after save/publish. |
...
| Application Review | Staff/Admin processes an authorized application. | Submitted application snapshot/documents, current state, permitted next transitions, rejection option, immutable status history; acceptance form with eligible-supervisor search/selection, required start/end dates, and optional note. | Move Submitted → Under Review → Interview → Accepted; reject from permitted state; add note; open created placement. | Non-terminal; terminal; transition validation/error; accepted placement created exactly once; no eligible supervisor blocks acceptance with an Admin contact message; identical retry shows existing placement, conflicting details show a conflict. | Applicant List/notification → Placement Overview after acceptance. |
...
| Placement Overview | Authorized participant sees core placement context. | Student, posting, company, assigned supervisor, dates, lifecycle, tabs for Tasks/Reports/Evaluations and role-appropriate summary. | Supervisor/Admin mark Completed or Terminated; Admin opens assignment history and replaces/revokes supervisor with a required reason and confirmation; open tabs. | Active; Completed/Terminated history; lifecycle confirmation/error; Admin sees temporarily unassigned state and handles reviews; assignment conflict refreshes current supervisor. | Placement list/application acceptance → tab views. |
...
| Report Review Detail | Supervisor/Admin reads original report and makes a human review decision. | Original content and attachments, version/review history, on-demand advisory AI summary, feedback field, decision controls bound to the displayed reportVersionId; summary explicitly labels that same version. | Approve; request revision with required feedback; request summary. | Submitted; Revision Requested history; Approved; AI loading/unavailable/error; feedback validation; stale-version 409 retains entered feedback, refreshes history and requires reread/reconfirmation; inactive placement read-only. | Review list → returns to queue or placement report history. |
...
4. Staff/Admin opens an application and uses only permitted transitions. Acceptance collects supervisor/start/end dates, then atomically retains the command/history, creates one placement with assignment and reporting periods, notifies the Student, and exposes the placement link.
...
## Contract details for corrected flows

- Role switch calls setActiveRole before changing the shell; success replaces the bearer token, clears role-dependent data, cancels in-flight requests, and navigates to the role landing page. A forbidden selection keeps the prior role; an invalidated session uses the normal sign-in recovery flow.
- Admin assignment management uses listSupervisorAssignments and listEligibleSupervisors(placementId), then changeSupervisorAssignment with the displayed expectedAssignmentId. Null supervisor revokes; a replacement is atomic. Company progress remains read-only.
- Document controls upload/download through the API content URLs with bearer authentication and show expiry/retry errors. A download uses authenticated fetch and a blob, not unauthenticated navigation. Reject unsupported formats or files over 10 MiB before upload; server validation remains authoritative.
- An empty posting skill set displays “No skills specified; score is not an assessment of suitability.” Score details explain required weight 2, optional weight 1, and proficiency-independent presence matching. Advisory AI shows source version/snapshot and a stale label when current match inputs change.
- Reporting calendar and monitoring display the same dueAt/timezone. A missing report reminder opens the placement's Reports tab with reportingPeriodId selected. Submitted history never reveals another user's draft attachment selection. See [behavior rules](../design/behavior-rules.md) and [acceptance scenarios](../design/traceability.md).
