...
Core user actions use synchronous REST requests. In-app notifications are inserted in the same transaction as their triggering application event, then exposed through the notification API; no external delivery provider is required. When a user requests AI help, the API authorizes the source data, creates an AI job, and returns without delaying the core workflow. The SPA can refresh generated-result status while the worker processes it.
...
| Placements and Progress | Placement lifecycle, task assignment/status ownership, weekly-report drafts/submissions/revisions, and retained report versions/review feedback. |
...
| AI Job Orchestration | Authorized, minimized requests for match explanations and report summaries; durable pending/succeeded/failed job state. |
