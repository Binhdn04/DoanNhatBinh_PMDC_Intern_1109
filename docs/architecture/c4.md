...
This is the canonical C4 overview for InternHub. It describes the implemented architecture that supports the requirements and UI flows in [`docs/requirements/requirements.md`](../requirements/requirements.md) and [`docs/ui/`](../ui/): an API-backed React/Vite client, NestJS modular monolith, PostgreSQL, private object storage, and shared packages. The optional AI worker is the only deferred container.

InternHub uses an implemented three-tier modular-monolith design. The optional AI worker shown in the diagrams remains a deferred extension; it is not deployed:

| Tier         | Responsibility                                                                                                                                         | Containers                    |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------- |
...
| Application  | Authenticates and authorizes requests, coordinates use cases, applies domain rules, creates audit/notification records, and controls data access.      | API; deferred AI worker       |
...
Dependencies flow from Presentation to Application to Data. The API deadline scheduler is part of the deployed application tier. If enabled later, the worker will perform optional AI jobs only and will not become a second source of domain truth.
