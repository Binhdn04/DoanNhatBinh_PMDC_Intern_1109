...
This is the PostgreSQL logical model for the modular-monolith API described in [C4](../architecture/c4.md). It supports the authenticated UI flows in [screens](../ui/screens.md) and requirements RQ-01–RQ-16. PostgreSQL is the authoritative store for business data, immutable history, notifications, and AI-job state. Private object storage holds document bytes only; it is not a database or an API authority.

The companion [`../data/internship_platform.dbml`](../data/internship_platform.dbml) and ERD are design views. The executable schema source is the TypeORM entities and ordered migrations under `apps/api/src/infrastructure/database/`. The implemented model uses multi-role users and company memberships, does not persist AI interviews or AI screening scores, and keeps self-assessments separate from performance evaluations.

## Conventions

- All identifiers are `uuid`; generated identifiers use `gen_random_uuid()`.
- All timestamps are `timestamptz`, stored in UTC. Product dates use an IANA timezone: term override, then its program, then `Asia/Ho_Chi_Minh`. Postings snapshot `deadline_timezone` at creation; placements snapshot `reporting_timezone` at acceptance. Later timezone configuration changes do not move existing deadlines. See [calendar rules](../design/behavior-rules.md).
...
erDiagram
  ...
  placements ||--o{ reporting_periods : schedules
  reporting_periods ||--o| weekly_reports : receives
  weekly_reports ||--o{ report_draft_documents : selects
  documents ||--o{ report_draft_documents : draft_attachment
...
| `auth_sessions` | `id`, `user_id`, `active_role`, `version integer`, `expires_at`, `revoked_at nullable` | JWT `sid` and session version must match this row on every request; role must still exist in user_roles. Role switch locks and updates only this session. |
| `programs` | `id`, `code varchar(50)`, `name varchar(200)`, `active boolean`, `timezone varchar(100) default 'Asia/Ho_Chi_Minh'` | unique `code`; provides monitoring scope. |
| `academic_terms` | `id`, `program_id`, `name varchar(100)`, `starts_on date`, `ends_on date`, `timezone varchar(100) nullable` | unique `(program_id, name)`; `ends_on >= starts_on`. |
...
| Table | Key columns and types | Keys, constraints, and purpose |
| --- | --- | --- |
| `postings` | `id`, `company_id`, `term_id nullable`, `title varchar(200)`, `description text`, `category varchar(100)`, `location varchar(200)`, `work_arrangement`, `duration_weeks smallint`, `openings integer`, `responsibilities text nullable`, `benefits text nullable`, `application_deadline date`, `deadline_timezone varchar(100)`, `skills_declared boolean default false`, `status posting_status`, `published_at timestamptz nullable`, timestamps | FK to company/term; `openings > 0`, `duration_weeks > 0`; status is `draft`, `open`, `closed`, or `archived`. Eligibility is derived: `status = open AND application_deadline >= current_product_date`; it is never stored as an independently mutable flag. |
| `posting_skills` | `posting_id`, `skill_id`, `importance skill_importance` | PK `(posting_id, skill_id)`; importance is `required` or `optional`. Both skill groups must be explicitly declared, but either or both may be empty. Saving a supplied skills array (including []) sets postings.skills_declared=true; publish rejects false. Duplicate normalized skills, including across groups, are rejected. No minimum group cardinality applies. |
...
| `applications` | `id`, `student_id`, `posting_id`, `status application_status`, `contact_name varchar(200)`, `contact_email citext`, `contact_phone varchar(40)`, `university varchar(200)`, `major varchar(200)`, `graduation_year smallint`, `availability text`, `cover_note text NOT NULL CHECK (length(trim(cover_note)) > 0)`, `cv_document_id uuid`, `submitted_at`, `updated_at` | unique `(student_id, posting_id)` including terminal applications; FK to student/posting/CV document; stores submission-time profile snapshot. Status: `submitted`, `under_review`, `interview`, `accepted`, `rejected`, `withdrawn`. No replacement application is possible. |
...
| `application_status_history` | `id`, `application_id`, `from_status nullable`, `to_status`, `actor_user_id`, `note text nullable`, `acceptance_command jsonb nullable`, `changed_at` | immutable PK `id`; index `(application_id, changed_at)`; first row has `from_status NULL`, `to_status submitted`. Write access is application service only. The accepted row retains canonical supervisorUserId, startDate, endDate and trimmed note (absent/blank becomes null) for replay comparison; unique partial application_id where to_status=accepted. |
| `placements` | `id`, `application_id`, `student_id`, `posting_id`, `company_id`, `term_id nullable`, `reporting_timezone varchar(100)`, `start_date date`, `end_date date`, `status placement_status`, `ended_at timestamptz nullable`, `ended_by_user_id uuid nullable`, timestamps | unique `application_id`; FKs to application/student/posting/company/term/user; `end_date >= start_date`; status `active`, `completed`, `terminated`. Acceptance transaction creates exactly one row, copying relationship context. Service verifies the application is accepted. |
| `placement_supervisor_assignments` | `id`, `placement_id`, `supervisor_user_id`, `assigned_at`, `revoked_at nullable`, `assigned_by_user_id`, `reason text nullable`, `revoked_by_user_id nullable`, `revocation_reason text nullable` | index `(placement_id, revoked_at)`; partial unique `(placement_id) WHERE revoked_at IS NULL`; a placement starts with one active assignment. Admin alone assigns/replaces/revokes after acceptance, on Active placements. Lock placement and compare expected active assignment ID; replace atomically. Revocation ends future supervisor access while retaining history. A revoked-only placement is temporarily unassigned; Admin retains review/lifecycle responsibility. |
...
| `reporting_periods` | `id`, `placement_id`, `week_start date`, `week_end date`, `due_at timestamptz` | Created at acceptance for every Monday-Sunday week overlapping placement dates; unique `(placement_id, week_start)` and `(id, placement_id)`; check ISO Monday and week_end=week_start+6. due_at is next Monday 00:00 in placement reporting_timezone. Exists even without a report. |
| `report_draft_documents` | `report_id`, `document_id`, `created_at` | PK `(report_id, document_id)`; FK report/document. Private draft attachment selection, replaced on save and copied into version links on submit. Blocks document deletion while referenced. |
| `weekly_reports` | `id`, `placement_id`, `reporting_period_id`, `state report_state`, `draft_accomplishments text nullable`, `draft_challenges text nullable`, `draft_next_week_plan text nullable`, `draft_saved_at timestamptz nullable`, `current_version_no integer default 0`, timestamps | unique `reporting_period_id`; composite FK `(reporting_period_id, placement_id)` to reporting_periods; week boundaries/due time are projected from the period; state `draft`, `submitted`, `revision_requested`, `approved`. draft_saved_at is set on draft save and cleared on submission, distinguishing an empty saved draft from no revision draft. Draft content and attachment selection are always private to the owning Student, including revisions. Other roles receive only submitted versions/reviews. Keep revision_requested while editing; submit changes it to submitted. |
...
| `report_reviews` | `id`, `report_version_id`, `outcome report_review_outcome`, `feedback text nullable`, `reviewed_by_user_id`, `reviewed_at` | FK version/user; `outcome` `approved` or `revision_requested`; check `outcome <> 'revision_requested' OR (feedback IS NOT NULL AND length(trim(feedback)) > 0)`; unique `report_version_id`; append-only. Service permits one final review per submitted version. |
...
| Table | Key columns and types | Keys, constraints, and purpose |
| --- | --- | --- |
| `notifications` | `id`, `recipient_user_id`, `type notification_type`, `title varchar(200)`, `body text nullable`, `target_type notification_target_type nullable`, `target_id uuid nullable`, `reporting_period_id uuid nullable`, `dedupe_key text nullable`, `created_at`, `read_at timestamptz nullable` | FK recipient; index `(recipient_user_id, read_at, created_at DESC)`; types include application status, task assigned, report feedback/revision, and deadline. Domain-event notifications are inserted in the same transaction as the source write; deadline notifications come from the API Notifications scheduler transaction. `dedupe_key text nullable UNIQUE` prevents duplicate reminder insertion. See calendar/reminder rules. No email/SMS/push outbox exists. |
...
| `ai_jobs` | `id`, `requested_by_user_id`, `kind ai_job_kind`, `posting_id nullable`, `report_version_id nullable`, `input_fingerprint char(64)`, `input_snapshot jsonb`, `prompt_version varchar(80)`, `claim_token uuid nullable`, `lease_expires_at timestamptz nullable`, `status ai_job_status`, `attempts smallint`, `available_at`, `claimed_at nullable`, `completed_at nullable`, `provider_reference varchar(200) nullable`, `generated_content text nullable`, `error_code varchar(80) nullable`, timestamps | job kind `match_explanation` or `report_summary`; check exactly one compatible source (`posting_id` for match, `report_version_id` for summary); status `pending`, `processing`, `succeeded`, `failed`; index `(status, available_at)` for worker claims and a partial unique `(requested_by_user_id, kind, input_fingerprint, prompt_version)` WHERE status IN (pending, processing, succeeded). Fingerprint includes source identity and immutable minimized snapshot. Snapshot is immutable and available only to the AI-job API module/worker, never exposed by the DTO. Lease-fenced claims, three attempts and interruption recovery follow behavior-rules.md. Only optional advisory content is stored here. |
...
| Application | Staff/Admin: `submitted -> under_review -> interview -> accepted`; rejection is allowed from any of those non-terminal states. Student: any non-terminal state to `withdrawn`. Terminal states reject new transitions. Under an application row lock, identical acceptance replays return existing data without writes; conflicting command input returns 409. Compare the retained original acceptance_command, not mutable current assignment. First acceptance creates placement, assignment, reporting periods, history and notification atomically; unique application_id backstops concurrency. |
...
| Report | One active aggregate per placement/week. Draft saves replace only draft fields and report_draft_documents; revision saves keep revision_requested. Submit/resubmit appends a `report_versions` row and advances `current_version_no`; copy attachment links, then clear draft fields/links in that same transaction; review records are append-only. Review requires the explicitly submitted reportVersionId to equal the current version under a report/placement lock; stale or already reviewed versions return 409 without notification. Revision requested must include feedback. The student may resubmit only after revision requested. |
...
All required application/profile/task text has a nonblank constraint in addition to NOT NULL. Request validation trims whitespace before persistence. Draft report fields may be empty, but submitted version fields must be nonblank.

Document download/upload authorization first resolves the owning link (student/application/report version), then evaluates the same record policy. The browser transfers bytes only through authenticated API content routes; five-minute API transfer tokens are not storage URLs. API rechecks access on every transfer; draft attachment access is owner-only.
...
- `tasks(placement_id, status, due_date)`, `weekly_reports(placement_id, reporting_period_id)`, `reporting_periods(placement_id, week_start DESC)`, `reporting_periods(due_at)`, `report_reviews(report_version_id, reviewed_at)`.
...
| --- | --- | --- |
| RQ-01 | users/roles, memberships, assignments | all protected operations; `GET /api/v1/me` |
| RQ-02 | student profile, skills, preferences, documents | `/api/v1/students/me/*`, `/api/v1/documents` |
| RQ-03–06 | companies, postings, posting skills, saved postings | `/api/v1/companies`, `/api/v1/postings`, saved and match routes |
| RQ-07–08 | applications, documents, immutable status history, placements | `/api/v1/applications`, status/withdraw routes |
| RQ-09–10 | placements, assignments, tasks | `/api/v1/placements`, lifecycle and task routes |
| RQ-11 | reports, versions, reviews, attachment links | `/api/v1/placements/{placementId}/reports` and review routes |
| RQ-12–13 | separate assessment tables | self-assessment and performance-evaluation routes |
| RQ-14 | derived PostgreSQL read model/query | `GET /api/v1/monitoring` only; no monitoring mutation route |
| RQ-15 | notifications | `/api/v1/notifications` |
| RQ-16 | `ai_jobs` | `/api/v1/ai-jobs`, result polling; no AI interview/screening route |

## Audit correction migration order

Add session versioning and timezone columns first; validate IANA names in the application. Backfill immutable posting/placement timezones before generating reporting periods. Map legacy week rows to canonical periods and resolve overlaps without dropping submitted history. Backfill draft attachment links only from verified legacy sources. Preserve historical incomplete applications in an access-restricted migration archive before enforcing mandatory nonblank fields. New AI jobs require input snapshots; legacy pending/processing jobs without reconstructible input become failed rather than being silently recomputed. Add reminder and review uniqueness after detecting existing duplicates. This is a migration design, not a claim that migrations have run.
