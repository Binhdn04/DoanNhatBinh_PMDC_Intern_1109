...
const run = (args) =>
  new Promise((resolve, reject) => {
    const child = spawn("pnpm", args, {
      stdio: "inherit",
      env: { ...process.env, BROWSER_COVERAGE: "1" },
    });
...
const map = coverage.createCoverageMap(
  JSON.parse(await readFile("apps/web/coverage/coverage-final.json", "utf8")),
);
for (const file of await readdir("apps/web/coverage-browser"))
  map.merge(
    JSON.parse(await readFile(`apps/web/coverage-browser/${file}`, "utf8")),
  );
...
const context = report.createContext({
  dir: "apps/web/coverage-combined",
  coverageMap: map,
});
