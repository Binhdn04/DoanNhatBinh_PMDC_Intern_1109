...
describe('database migrations', () => {
  it.each([
    ...
    [new CanonicalWorkflows1770000001000(), ['reporting_periods', 'weekly_reports', 'ai_jobs']],
    [new WorkflowHardening1770000002000(), ['application_documents', 'UNIQUE']],
    [new CompleteCanonicalSchema1770000003000(), ['user_roles', 'posting_skills', 'application_status_history']],
  ...
  it('provides down SQL for reversible migrations', async () => {
    for (const migration of [new InitialSchema1770000000000(), new CanonicalWorkflows1770000001000(), new WorkflowHardening1770000002000()]) {
      const q = runner(); await migration.down(q);
      expect(q.query).toHaveBeenCalledWith(expect.stringContaining('DROP TABLE'));
    }
  });

  it('protects the forward-only cutover from reversal', async () => {
    await expect(new CompleteCanonicalSchema1770000003000().down()).rejects.toThrow('Forward-only canonical cutover');
  });
});
