...
export class PrivateStorageService implements OnModuleInit {
  ...
  putStream(key: string, stream: Readable, size: number, contentType: string) { return this.client.putObject(this.bucket, key, stream, size, { 'Content-Type': contentType }); }
  ...
  async verify(key: string) {
    const stream = await this.get(key); const hash = createHash('sha256'); const first: Buffer[] = []; let firstSize = 0; let size = 0;
    for await (const chunk of stream) { const bytes = Buffer.from(chunk); size += bytes.length; hash.update(bytes); if (firstSize < 8) { const head = bytes.subarray(0, 8 - firstSize); first.push(head); firstSize += head.length; } }
    return { size, sha256: hash.digest('hex'), signature: Buffer.concat(first) };
  }
}
