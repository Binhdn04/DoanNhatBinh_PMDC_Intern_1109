...
export function reportingPeriods(start: string, end: string, timezone: string) {
  ...
  const monday = new Date(startDate); monday.setUTCDate(monday.getUTCDate() - ((monday.getUTCDay() + 6) % 7));
  const rows: { weekStart: string; weekEnd: string; dueAt: Date }[] = [];
  for (let week = new Date(monday); week <= endDate; week.setUTCDate(week.getUTCDate() + 7)) {
    const dueLocal = new Date(week); dueLocal.setUTCDate(dueLocal.getUTCDate() + 7);
    rows.push({ weekStart: iso(week), weekEnd: iso(addDays(week, 6)), dueAt: localMidnightUtc(iso(dueLocal), timezone) });
  }
  return rows;
}
export function iso(date: Date) { return date.toISOString().slice(0, 10); }
export function addDays(date: Date, days: number) { const next = new Date(date); next.setUTCDate(next.getUTCDate() + days); return next; }
export function localMidnightUtc(date: string, timezone: string) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    const get = (kind: string) => parts.find((p) => p.type === kind)?.value ?? '00';
    const seen = Date.parse(`${get('year')}-${get('month')}-${get('day')}T${get('hour') === '24' ? '00' : get('hour')}:${get('minute')}:00Z`);
